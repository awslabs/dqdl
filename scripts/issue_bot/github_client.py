...
class GitHubClient:
    ...
    def get_codebase_map(self, src_dir="src/com"):
        ...
        try:
            proc = subprocess.run(
                ["find", full_dir, "-name", "*.java", "-not", "-path", "*/tst/*"],
    ...
    def read_local_file(self, path):
        if not path.startswith("src/") and not path.startswith("tst/") and not path.startswith("configuration/"):
